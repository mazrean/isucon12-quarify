...
func authorizePlayer(ctx context.Context, tenantDB dbOrTx, tenantId int64, id string) error {
	player, err := retrievePlayerCache(ctx, tenantDB, tenantId, id)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func billingReportByCompetition(ctx context.Context, tenantDB dbOrTx, tenantID int64, competitonID string) (*BillingReport, error) {
	...
	if err := adminDB.SelectContext(
		ctx,
		&vhs,
		"SELECT player_id, created_at AS min_created_at FROM visit_history2 WHERE tenant_id = ? AND competition_id = ? GROUP BY player_id",
		tenantID,
		comp.ID,
...
func competitionScoreHandler(c echo.Context) error {
	...
	for {
		...
		if _, err := retrievePlayerCache(ctx, tenantDB, v.tenantID, playerID); err != nil {
			// 存在しない参加者が含まれている
			if errors.Is(err, sql.ErrNoRows) {
				return echo.NewHTTPError(
					http.StatusBadRequest,
...
func playerHandler(c echo.Context) error {
	...
	p, err := retrievePlayerCache(ctx, tenantDB, v.tenantID, playerID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return echo.NewHTTPError(http.StatusNotFound, "player not found")
		}
	...
	query, args, err := sqlx.In(
		"SELECT ps.* FROM player_score AS ps WHERE ps.tenant_id = ? AND ps.player_id = ? AND ps.id = (SELECT ps2.id FROM player_score AS ps2 WHERE ps2.tenant_id = ps.tenant_id AND ps2.player_id = ps.player_id ORDER BY ps2.row_num DESC LIMIT 1)",
		v.tenantID, playerID,
	)
	if err != nil {
...
func competitionRankingHandler(c echo.Context) error {
	...
	if _, err := adminDB.ExecContext(
		ctx,
		"INSERT IGNORE INTO visit_history2 (player_id, tenant_id, competition_id, created_at) VALUES (?, ?, ?, ?)",
	...
	if err := tenantDB.SelectContext(
		ctx,
		&pss,
		"SELECT * FROM player_score AS ps WHERE ps.tenant_id = ? AND ps.competition_id = ? AND ps.id = (SELECT ps2.id FROM player_score AS ps2 WHERE ps2.tenant_id = ps.tenant_id AND ps2.competition_id = ps.competition_id AND ps2.player_id = ps.player_id ORDER BY row_num DESC LIMIT 1) ORDER BY score DESC, row_num LIMIT 100 OFFSET ?",
		tenant.ID,
		competitionID,
		rankAfter,
	); err != nil {
	...
	for _, ps := range pss {
		...
		p, err := retrievePlayerCache(ctx, tenantDB, v.tenantID, ps.PlayerID)
		if err != nil {
			return fmt.Errorf("error retrievePlayer: %w", err)
		}
...
func meHandler(c echo.Context) error {
	...
	ctx := context.Background()
	p, err := retrievePlayerCache(ctx, tenantDB, v.tenantID, v.playerID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return c.JSON(http.StatusOK, SuccessResult{
				Status: true,
